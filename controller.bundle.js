window.getDrawParams = function() {
	return {
		...
		draw: [
			{ func: (x) => { return Math.cos(x + Math.PI / 2 ) }, x: { min: -Math.PI, max: Math.PI }, forx: 58, color: [0.0,0.0,1.0,1.0] },
			{ func: (x) => { return Math.cos(x + Math.PI / 3 ) }, x: { min: -Math.PI, max: Math.PI }, forx: 58, color: [0.0,0.0,1.0,1.0] },
