...
export async function searchWordsMain(value, scale, fields) {
  try {
    if (value.length >= 3) {
      const response = await search.api.apiSearchWordsMain(value, scale, fields);
      const responseWithNewUri = changeArrayUriForGetLinearization(response.data);
