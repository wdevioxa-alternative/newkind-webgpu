        ...
        <TreeView
            ...
            onSelect={({element, isBranch, isExpanded, isSelected, isHalfSelected, isDisabled, treeState }) => {
                if(isSelected && !isLoading_toc && !isLoading_page) {
                    ...
                    switch (element?.classKind?.toLowerCase()) {
                        case "window":
                        case "category":
                        case "chapter":
                            action = element.code
                            break
                        case "block":
                            action = element.codeRange
                            break
                        default:
                            console.error('Неизвестный тип classKind', element)
                            if(element.code) {
                                action = element.code
                            } else if(element.codeRange) {
                                action = element.codeRange
                            } else {
                                console.error('Нет code и codeRange записываю title', element)
