 ...
 * @constructor
 */
export const Editor = ({type, postcoordinationTree, postcoordination, className, title, root, isRoot, forceUpdate, setForceUpdate, allToc, setAllToc }) => {
    const rndRef = useRef(null);
    const treeRefs = useRef(new Array())
    ...
    useEffect(() => {
        treeRefs.current.forEach(item => {
            if(item !== null) {
                // console.log('@@@@@@@@@@@@@@@@@@@@@@@@@', item?.dataset?.code, postcoordinationTree.request.codes)
                // if(postcoordinationTree.request.codes.has(item?.dataset?.code)) {
                //     item.click()
                // }
            }
        })
    ...
    useEffect(() => {
        const setData = async () => {
            ...
            for(let item in postcoordinationTree.actions.onSetCode) {
                if(isRoot && !postcoordinationTree.actions.onSetCode[item].isSecond) {
                    await onSetCode(postcoordinationTree.actions.onSetCode[item].axis, postcoordinationTree.actions.onSetCode[item].element)
                    delete postcoordinationTree.actions.onSetCode[item]
                }
                            ...
                            {axis.isVisible ? (
                                <React.Fragment key={idKey()} >
                                    <div className={style.editor}>
                                        {axis.scaleEntity.map((object, index) => {
