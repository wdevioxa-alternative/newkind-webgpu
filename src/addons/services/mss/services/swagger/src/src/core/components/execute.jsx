...
export default class Execute extends Component {
  ...
  handleValidationResultPass = async () => {
  ...
  onClick = () => {
    (getStore().api).execute[`${this.props.path}`][`${this.props.method}`].onClick.callback(this)
  ...
  render(){
    ...
    api.execute[`${this.props.path}`] = {
      path: this.props.path,
      method: this.props.method
    }

    api.execute[`${this.props.path}`][`${this.props.method}`].onClick.swagger = this.onClick

    return (
        <button className="btn execute opblock-control__btn" onClick={ api.execute[`${this.props.path}`][`${this.props.method}`].onClick.service } disabled={disabled}>
          Execute
        </button>
    )
  }
