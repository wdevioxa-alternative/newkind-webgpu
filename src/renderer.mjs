...
export class GApplication
{
    ...
    async initializeResources() {
        ...
        this.texturePipeline = this.device.createRenderPipeline(descriptor);
...
*/
        ...
        for ( let i = 0; i < objectparam.draw.length; i++ ) 
        {
            if ( objectparam.draw[i].coords.visibility ) {
                await this.spline.axisDraw( this, objectparam.draw[i].coords.x.min, objectparam.draw[i].coords.x.max,
                    objectparam.draw[i].coords.x.repeat, objectparam.draw[i].coords.y.min, objectparam.draw[i].coords.y.max,
                    objectparam.draw[i].coords.y.repeat, objectparam.draw[i].coords.color );
		break;
            }
        }
        for ( let i = 0; i < objectparam.draw.length; i++ ) 
	    ...
	    let drawpoints = false;
            ...
            if ( objectparam.draw[i].range ) {
                await this.spline.functionDraw( this, objectparam.draw[i].range.min, objectparam.draw[i].range.max, objectparam.draw[i].range.repeat, objectparam.draw[i].func, drawpoints, objectparam.draw[i].color );
            } else {
